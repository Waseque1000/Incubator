...
function HelperText({ children }) {
  return <p className="text-xs sm:text-sm text-gray-500 mt-1.5">{children}</p>;
}
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-emerald-50 to-teal-50 px-4 py-6 sm:px-6 md:px-10 md:py-10">
      <div className="max-w-5xl mx-auto">
        {/* Header Section */}
        <div className="text-center mb-8 sm:mb-10">
          <div className="inline-flex items-center gap-3 bg-white/70 backdrop-blur-md px-4 py-2 sm:px-6 sm:py-3 rounded-2xl shadow-lg mb-5 sm:mb-6">
            <Calendar className="w-5 h-5 text-emerald-600" />
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold bg-gradient-to-r from-emerald-600 to-teal-600 bg-clip-text text-transparent mb-3 sm:mb-4">
            Daily Progress Update
          </h1>

          <p className="text-base sm:text-lg md:text-xl text-gray-600 flex items-center justify-center gap-2">
            <UserCheck className="w-5 h-5 sm:w-6 sm:h-6 text-emerald-500" />
            Share your today's achievement
          </p>
        </div>

        {/* Form Card */}
        <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-gray-200/50 p-5 sm:p-7 md:p-10 lg:p-12">
          <form onSubmit={handleSubmit} className="space-y-7 sm:space-y-8">
            ...
            <div className="grid md:grid-cols-2 gap-5 sm:gap-7">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleChange}
                  placeholder="Enter your full name"
                  className="mt-2 w-full rounded-2xl border-2 border-gray-300 px-4 py-3 sm:px-5 sm:py-4 text-sm sm:text-base outline-none transition-all focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/20"
                />
              </div>
            ...
            <div>
              ...
              <div className="relative mt-2">
                <select
                  ...
                  value={form.module}
                  onChange={handleChange}
                  className="w-full appearance-none rounded-2xl border-2 border-gray-300 bg-white px-4 py-3 sm:px-5 sm:py-4 pr-12 text-sm sm:text-base outline-none transition-all focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/20"
                >
                  <option value="">Choose a module...</option>
                  {moduleOptions.map((m) => (
                    <option key={m} value={m}>
                ...
                <ChevronDown className="pointer-events-none absolute right-4 sm:right-5 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-500" />
              </div>
            </div>

            {/* Guidelines Checkbox + Submit */}
            <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 sm:gap-6 pt-2 sm:pt-4">
              <label className="flex items-center gap-3 sm:gap-4 rounded-2xl border-2 border-gray-300 px-4 py-4 sm:px-6 sm:py-5 cursor-pointer select-none hover:border-emerald-400 hover:bg-emerald-50 transition-all w-full md:w-auto">
                <input
                  type="checkbox"
                  name="needGuidelines"
                  checked={form.needGuidelines}
                  onChange={handleChange}
                  className="h-5 w-5 sm:h-6 sm:w-6 rounded border-gray-400 text-emerald-600 focus:ring-emerald-500"
                />
                <div>
                  <div className="font-semibold text-gray-800 flex items-center gap-2 text-sm sm:text-base">
                    <Sparkles className="w-5 h-5 text-emerald-600" />
                    Need Guidelines or Help?
                  </div>
              ...
              <button
                type="submit"
                disabled={submitting}
                className={`w-full md:w-auto justify-center rounded-2xl px-6 sm:px-8 md:px-10 py-4 sm:py-5 font-bold text-base sm:text-lg transition-all flex items-center gap-3 shadow-xl ${
                  submitting
                    ? "bg-emerald-600 text-white cursor-not-allowed"
                    : "bg-gradient-to-r from-emerald-500 to-teal-600 text-white hover:from-emerald-600 hover:to-teal-700 active:scale-95 shadow-emerald-500/40"
                }`}
            ...
            {msg.text && (
              <div
                className={`rounded-2xl px-4 sm:px-6 py-4 sm:py-5 text-sm sm:text-base font-medium flex items-center gap-3 sm:gap-4 shadow-inner ${
                  msg.type === "success"
                    ? "bg-emerald-50 border-2 border-emerald-200 text-emerald-800"
                    : "bg-red-50 border-2 border-red-200 text-red-800"
                }`}
